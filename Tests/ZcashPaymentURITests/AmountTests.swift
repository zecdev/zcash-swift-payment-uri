...
struct AmountTests {
    @Test func amountStringDecimals() throws {
        #expect(try LegacyAmount(value: 123.456).toString() == "123.456")

        #expect("\(try LegacyAmount(value: 123.456))" == "123.456")

        let stringDecimal = try LegacyAmount(string: "123.456")
        let literalDecimal = try LegacyAmount(value: 123.456)
        #expect(stringDecimal == literalDecimal)
    }

    @Test func amountTrailing() throws {
        #expect(try LegacyAmount(value: 50.000).toString() == "50")
    }

    @Test func amountLeadingZeros() throws {
        #expect(try LegacyAmount(value: 0000.5).toString() == "0.5")
    }

    @Test func amountMaxDecimals() throws {
        #expect(try LegacyAmount(value: 0.12345678).toString() == "0.12345678")
    }

    @Test func amountThrowsIfMaxSupply() throws {
        #expect(throws: (any Error).self) {
            try LegacyAmount(decimal: Decimal(21_000_000.00000001)).toString()
        }
        #expect(throws: (any Error).self) {
            try LegacyAmount(value: 21_000_000.00000001).toString()
        }
        #expect(throws: (any Error).self) {
            try LegacyAmount(string: "21_000_000.00000001").toString()
        }
    }

    @Test func amountThrowsIfNegativeAmount() throws {
        #expect(throws: (any Error).self) {
            try LegacyAmount(value: -1).toString()
        }
    }
    ...
    @Test func negativeAmountsAreRejectedEagerlyOnEveryPath() throws {
        ...
        for negative in ["-1", "-0", "-0.5", "-21000001", "-0.123456789", "-", "-."] {
            #expect(throws: LegacyAmount.AmountError.negativeAmount, "for input \(negative)") {
                try LegacyAmount(string: negative)
            }
        }

        // an explicit `+` sign is equally outside the grammar (but is not "negative").
        #expect(throws: LegacyAmount.AmountError.invalidTextInput) {
            try LegacyAmount(string: "+1")
        }

        // Double path.
        #expect(throws: LegacyAmount.AmountError.negativeAmount) {
            try LegacyAmount(value: -0.00000001)
        }

        // Decimal path.
        #expect(throws: LegacyAmount.AmountError.negativeAmount) {
            try LegacyAmount(decimal: Decimal(-1))
        }
    }
    ...
    func malformedDecimalStringsAreRejected(malformed: String) throws {
        #expect(throws: LegacyAmount.AmountError.invalidTextInput, "for input \(malformed)") {
            try LegacyAmount(string: malformed)
        }
    }
    ...
    func nonFiniteDoublesAreRejected(nonFinite: Double) throws {
        #expect(throws: LegacyAmount.AmountError.invalidTextInput) {
            try LegacyAmount(value: nonFinite)
        }
    }

    /// The internal zero constant renders canonically and equals a parsed zero.
    @Test func zeroAmount() throws {
        #expect(LegacyAmount.zero.toString() == "0")
        #expect(try LegacyAmount.zero == LegacyAmount(string: "0"))
        #expect(try LegacyAmount.zero == LegacyAmount(string: "0.0"))
    }
    ...
    @Test func decimalInitWithEagerRounding() throws {
        ...
        #expect(try LegacyAmount(decimal: inRange, rounding: true).toString() == "0.12345678")

        let tooPrecise = try #require(Decimal(string: "0.123456789"))
        #expect(throws: LegacyAmount.AmountError.tooManyFractionalDigits) {
            try LegacyAmount(decimal: tooPrecise, rounding: true)
        }
    }
    ...
    @Test func amountThrowsIfTooManyFractionalDigits() throws {
        #expect(throws: (any Error).self) {
            try LegacyAmount(string: "0.123456789")
        }
    }

    @Test func amountParsesMaxFractionalDigits() throws {
        #expect(try LegacyAmount(string: "0.12345678").toString() == (try LegacyAmount(value: 0.12345678).toString()))
    }

    @Test func amountParsesMaxAmount() throws {
        #expect(try LegacyAmount(string: "21000000").toString() == (try LegacyAmount(value: 21_000_000).toString()))
    }
    ...
    @Test func doubleToDecimal() throws {
        ...
        let amount = try LegacyAmount(value: 10_000.00002)
    ...
    func fractionsOfZecFromDouble(value: Double, expected: String) throws {
        #expect(try LegacyAmount(value: value).toString() == expected)
    }

    @Test func tooManyFractionsThrows() throws {
        // more digits than supposed to
        #expect(throws: (any Error).self) {
            try LegacyAmount(decimal: Decimal(0.000000002)).toString()
        }
        #expect(throws: (any Error).self) {
            try LegacyAmount(decimal: Decimal(10_000_000.000000002))
        }
    }
}
